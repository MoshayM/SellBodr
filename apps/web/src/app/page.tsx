...
/* ─── Opportunity card (desktop hero only) ───────────────── */
function OpportunityCard({ card, delay }: { card: typeof CARDS[0]; delay: number }) {
  ...
  return (
    <div ref={ref} onMouseMove={onMove} onMouseLeave={() => setTilt({ x: 0, y: 0 })} style={{ perspective: '800px' }}>
      <motion.div
        ...
        className="bg-white rounded-2xl p-4 w-52 select-none cursor-default border border-slate-200"
      >
          ...
          <div className="flex items-center gap-1.5">
            <div className="h-1.5 w-16 bg-slate-100 rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full rounded-full" style={{ background: `linear-gradient(90deg,${card.color},#6366F1)` }}
              />
            </div>
...
export default function LandingPage() {
  ...
  /* Geo / pricing */
  useEffect(() => {
    fetch('/api/v1/geo').then(r => r.json()).then((g: any) => {
      if (g.country === 'IN') { setCurrency('INR'); setProINR(99); }
      else {
        setCurrency('USD');
        fetch('/api/v1/platform/settings').then(r => r.json())
  ...
  const plans = [
    {
      key: 'free', name: 'Starter', priceINR: 0, priceUSD: 0,
    ...
    {
      key: 'pro', name: 'Pro', priceINR: proINR, priceUSD: Number(proPrice),
  ...
  return (
    <div className="w-full">
      ...
      {/* ── Navbar ─────────────────────────────────────────────── */}
      <nav
        className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between h-16 px-4 sm:px-6 lg:px-12"
        style={{
        ...
        <Link href="/" className="flex items-center gap-2.5 shrink-0">
          ...
          <div>
            <div className="text-[14px] font-black tracking-tight leading-none text-white">SellBodr</div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link href="/login" className="hidden sm:block text-sm font-medium hover:text-white transition-colors duration-150"
            style={{ color: 'rgba(255,255,255,0.6)' }}>Sign in</Link>
          <Link href="/register" className="btn-scout text-sm px-4 py-2">Get started →</Link>
          {/* Hamburger */}
          <button
            className="md:hidden flex flex-col justify-center items-center gap-[5px] w-9 h-9 rounded-lg ml-1"
            onClick={() => setMenuOpen(v => !v)} aria-label={menuOpen ? 'Close menu' : 'Open menu'}>
            <span className={`block w-5 h-0.5 rounded-full bg-white/70 transition-all duration-200 origin-center ${menuOpen ? 'rotate-45 translate-y-[7px]' : ''}`} />
            <span className={`block w-5 h-0.5 rounded-full bg-white/70 transition-all duration-200 ${menuOpen ? 'opacity-0 scale-x-0' : ''}`} />
            <span className={`block w-5 h-0.5 rounded-full bg-white/70 transition-all duration-200 origin-center ${menuOpen ? '-rotate-45 -translate-y-[7px]' : ''}`} />
          </button>
        </div>
      </nav>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div key="mobile-nav"
            ...
            style={{ background: 'rgba(10,21,43,0.98)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 16px 40px rgba(0,0,0,0.35)' }}>
            {NAV_LINKS.map(item => (
              <a key={item.label} href={item.href} onClick={() => setMenuOpen(false)}
                className="px-6 py-4 text-[15px] font-medium border-b active:bg-white/5 transition-colors"
            ...
            <div className="flex gap-3 px-6 py-5">
              <Link href="/login" onClick={() => setMenuOpen(false)}
                ...
                style={{ color: 'rgba(255,255,255,0.65)', borderColor: 'rgba(255,255,255,0.15)' }}>Sign in</Link>
              <Link href="/register" onClick={() => setMenuOpen(false)}
                className="flex-1 btn-scout text-sm py-3 text-center rounded-xl">Get started →</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* ── Hero ───────────────────────────────────────────────── */}
      {/*
        IMPORTANT: plain <section>, no motion transforms, no absolute orbs on mobile.
        overflow:hidden clips any desktop-only decorations.
      */}
      <section
        className="relative w-full min-h-screen flex flex-col items-center justify-center text-center overflow-hidden pt-20 pb-12 px-4 sm:px-6"
        style={{ background: 'linear-gradient(180deg,#0D1B35 0%,#111d38 18%,#162240 36%,#1c2d50 52%,#243460 74%,#ddd8ff 88%,#EEF2FF 94%,#FFFFFF 100%)' }}>

        {/* Dot grid — covers only the dark zone, no overflow risk */}
        <div className="absolute top-0 left-0 right-0 h-3/5 pointer-events-none"
        ...
        {/* Ambient orbs — DESKTOP ONLY, positioned inward so they never overflow */}
        <div className="hidden sm:block absolute top-[8%] left-[10%] w-80 h-80 rounded-full blur-3xl pointer-events-none"
          style={{ background: 'radial-gradient(circle,rgba(99,102,241,0.2),transparent 70%)' }} />
        <div className="hidden sm:block absolute top-[10%] right-[10%] w-64 h-64 rounded-full blur-3xl pointer-events-none"
          style={{ background: 'radial-gradient(circle,rgba(124,58,237,0.16),transparent 70%)' }} />
        <div className="hidden sm:block absolute bottom-[30%] left-[15%] w-72 h-72 rounded-full blur-3xl pointer-events-none"
          style={{ background: 'rgba(167,139,250,0.15)' }} />
        <div className="hidden sm:block absolute bottom-[25%] right-[15%] w-72 h-72 rounded-full blur-3xl pointer-events-none"
        ...
        <div className="relative z-10 w-full max-w-3xl mx-auto flex flex-col items-center">
          ...
          <motion.div
            ...
            style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.18)', color: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
            <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse shrink-0" />
            <span>AI-powered · 76+ marketplaces · Live data</span>
          </motion.div>

          {/* Headline */}
          <motion.h1
            initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7 }}
            className="text-4xl sm:text-5xl lg:text-7xl font-black leading-[1.08] tracking-tight mb-5 w-full"
          ...
          <motion.p
            ...
            style={{ color: 'rgba(255,255,255,0.86)', textShadow: '0 1px 10px rgba(0,0,0,0.45)' }}>
            Source in India. Sell on Amazon, Etsy &amp; 74+ marketplaces — with AI scoring, a full profit model, verified suppliers on a live map, and a verdict in under 60 seconds.
          </motion.p>

          {/* CTA */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.62 }}
            className="w-full flex flex-col sm:flex-row items-center justify-center gap-3 mb-4">
            <Link href="/register"
              className="btn-scout w-full sm:w-auto text-sm sm:text-base px-8 py-3.5 rounded-2xl text-center"
              style={{ boxShadow: '0 8px 32px rgba(99,102,241,0.55)' }}>
          ...
          <div className="hidden sm:flex items-end justify-center gap-5 flex-wrap">
            {CARDS.map((c, i) => (
              <div key={c.product} className={i === 0 ? 'sm:mb-6' : i === 2 ? 'sm:mb-3' : ''}>
                <OpportunityCard card={c} delay={0.9 + i * 0.15} />
              </div>
            ))}
          </div>

          {/* Mobile social proof — simple stat pills instead of cards */}
          <div className="flex sm:hidden items-center justify-center gap-3 flex-wrap">
            {[['91', 'Brass Diyas — Amazon US'], ['87', 'Pashmina — Etsy UK'], ['83', 'Coasters — Amazon DE']].map(([score, label]) => (
              <div key={label} className="flex items-center gap-2 rounded-full px-3 py-1.5 text-xs"
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2"
          ...
          <motion.div animate={{ y: [0, 6, 0] }} transition={{ repeat: Infinity, duration: 1.5 }}
            className="w-px h-6" style={{ background: 'linear-gradient(to bottom,rgba(255,255,255,0.35),transparent)' }} />
        </div>
      </section>

      {/* ── Stats strip ────────────────────────────────────────── */}
      <section className="py-14 px-4 sm:px-6" style={{ background: '#0D1B35', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="w-full max-w-5xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {STATS.map((s, i) => (
            <motion.div key={s.label}
              ...
              <div className="text-[10px] sm:text-[11px] font-medium leading-snug" style={{ color: 'rgba(255,255,255,0.42)' }}>{s.label}</div>
            </motion.div>
          ))}
        </div>
      </section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {FEATURES.map((f, i) => (
              <motion.div key={f.title}
                ...
                className="quick-tile p-5 sm:p-6 cursor-default">
      ...
      {/* ── How it works ───────────────────────────────────────── */}
      <section id="how" className="py-20 sm:py-24 px-4 sm:px-6 bg-slate-50">
          ...
          <div className="grid md:grid-cols-3 gap-4 sm:gap-5">
            {STEPS.map((s, i) => (
              <motion.div key={s.n}
                ...
                className="relative bg-white rounded-2xl p-5 sm:p-6 border border-slate-200 shadow-sm">
                <div className="absolute top-0 left-0 right-0 h-[2px] rounded-t-2xl"
                  style={{ background: 'linear-gradient(90deg,transparent,rgba(99,102,241,0.5),transparent)' }} />
                <div className="text-3xl sm:text-4xl font-black mb-3 leading-none text-gradient">{s.n}</div>
      ...
      {/* ── Pricing ────────────────────────────────────────────── */}
      <section id="pricing" className="py-20 sm:py-24 px-4 sm:px-6 bg-white">
        <div className="w-full max-w-5xl mx-auto">
            ...
            <div className="flex items-center gap-1 bg-slate-100 rounded-full p-1">
              {(['INR', 'USD'] as const).map(c => (
                <button key={c} onClick={() => setCurrency(c)}
                  className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-all duration-200 ${currency === c ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>
                  {c === 'INR' ? '₹ INR' : '$ USD'}
                </button>
              ))}
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-5 max-w-3xl mx-auto">
            {plans.map((p, i) => {
              const base = currency === 'INR' ? p.priceINR : p.priceUSD;
              const eff  = p.priceINR === 0 ? 0 : (isAnnual ? Math.round(base * 0.8) : base);
              const displayed = p.priceINR === 0
                ? (currency === 'INR' ? '₹0' : '$0')
                : (currency === 'INR' ? `₹${eff.toLocaleString('en-IN')}` : `$${eff}`);
              const saving = isAnnual && p.priceINR > 0
                ? (currency === 'INR' ? `Save ₹${Math.round(p.priceINR * 0.2 * 12).toLocaleString('en-IN')}/yr` : `Save $${Math.round(p.priceUSD * 0.2 * 12)}/yr`)
                : null;
              return (
                <motion.div key={p.key}
                  initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  className={`relative bg-white rounded-2xl p-6 sm:p-7 flex flex-col border-2 ${p.highlight ? 'border-violet-400 shadow-xl shadow-violet-100 pt-8 sm:pt-9' : 'border-slate-200 shadow-sm'}`}>
                  ...
                  <div className={`text-xs font-bold uppercase tracking-widest mb-2 ${p.highlight ? 'text-violet-600' : 'text-slate-500'}`}>{p.name}</div>
                  <div className="flex items-baseline gap-1 mb-1">
                    <span className="text-4xl font-black text-slate-900">{displayed}</span>
                    {p.priceINR > 0 && <span className="text-slate-400 text-sm">/mo</span>}
                  </div>
                  {saving
                    ? <div className="text-xs text-emerald-600 font-semibold mb-2">{saving}</div>
                  ...
                  <Link href={p.ctaHref}
                    className={p.highlight
                      ? 'btn-scout text-sm py-3 text-center w-full rounded-xl'
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            className="text-center text-slate-400 text-xs mt-5">
      ...
      <section className="py-14 sm:py-16 px-4 sm:px-6 bg-slate-50">
        <div className="w-full max-w-4xl mx-auto bg-white rounded-3xl p-6 sm:p-10 text-center border border-slate-200 shadow-sm">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="text-4xl mb-4">📱</div>
            <h2 className="text-2xl sm:text-3xl font-black mb-2 text-slate-900">Available on every device</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              {installPrompt ? (
                <button onClick={handleInstall}
                  className="bg-slate-50 hover:bg-violet-50 border border-slate-200 hover:border-violet-300 rounded-xl px-4 py-3 flex items-center gap-3 text-sm text-slate-600 hover:text-violet-700 transition-all touch-manipulation">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-center">
              <a href="https://github.com/MoshayM/SellBodr/releases/latest/download/SellBodr-Setup.exe"
                className="bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-300 rounded-xl px-4 py-3 flex items-center gap-3 text-sm text-slate-600 hover:text-blue-700 transition-all" download>
                <span className="text-2xl">🪟</span>
                <div className="text-left">
            ...
            {isInstalled && <p className="mt-4 text-sm text-emerald-600 font-medium">✓ SellBodr is installed on this device</p>}
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 sm:py-28 px-4 sm:px-6 text-center relative overflow-hidden"
        ...
        <div className="hidden sm:block absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full pointer-events-none"
          style={{ background: 'radial-gradient(circle,rgba(99,102,241,0.2),transparent 70%)' }} />
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black mb-5 leading-tight text-white">
            Start scouting{' '}
            <span style={{ background: 'linear-gradient(135deg,#a5b4fc 0%,#c4b5fd 50%,#f9a8d4 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              today
            </span>
          </h2>

          <p className="text-base sm:text-lg mb-8 leading-relaxed" style={{ color: 'rgba(255,255,255,0.62)' }}>
            AI-powered intelligence to source in India and sell on the world&apos;s top marketplaces.
          </p>

          <Link href="/register" className="btn-scout text-base sm:text-lg px-8 sm:px-10 py-4 sm:py-5 rounded-2xl inline-flex"
            style={{ boxShadow: '0 8px 32px rgba(99,102,241,0.55)' }}>
            Start scouting →
          </Link>
          ...
          <div className="mt-12 grid grid-cols-3 gap-3 max-w-xs mx-auto">
            {[['76+','Marketplaces'],['< 60s','First score'],['5 free','Scans']].map(([v, l]) => (
      ...
      {/* ── Footer ─────────────────────────────────────────────── */}
      <footer className="py-8 sm:py-10 px-4 sm:px-6" style={{ background: '#0A1525', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="w-full max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 sm:gap-6 text-sm flex-wrap justify-center" style={{ color: 'rgba(255,255,255,0.45)' }}>
