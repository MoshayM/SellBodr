  ...
  return (
    <div>
      <div className="mb-6 animate-card-in">
        ...
        <h1 className="text-2xl font-black text-white">Bulk Scan</h1>
        <p className="text-white/50 text-sm mt-1">Scan multiple products at once — up to 20 keywords per run</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        ...
        <div className="md:col-span-2 card-dark p-4 space-y-3 animate-card-in stagger-1">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs font-semibold ${tooMany ? 'text-rose-500' : keywords.length > 15 ? 'text-amber-500' : 'text-white/40'}`}>
              {keywords.length} / 20
            </span>
          </div>
          <textarea
            ...
            rows={10}
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white/80 placeholder-white/30 outline-none focus:border-indigo-400 transition-colors resize-none font-mono"
          />
        ...
        <div className="space-y-3">
          <div className="card-dark p-4 animate-card-in stagger-2">
            <label className="text-xs text-white/50 font-semibold mb-1.5 block">Target Marketplace</label>
          ...
          <div className="card-dark p-4 text-xs text-white/50 space-y-1.5 animate-card-in stagger-3">
      ...
      {scan.isPending && (
        <div className="card-dark p-12 text-center animate-card-in">
          ...
          <p className="text-white/40 text-sm mb-5">Running AI scan on {keywords.length} product{keywords.length !== 1 ? 's' : ''}</p>
          <div className="max-w-xs mx-auto h-1.5 rounded-full bg-white/8 overflow-hidden">
      ...
      {!scan.isPending && results.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between mb-1">
            <div className="text-sm text-white/50 font-medium">{results.length} result{results.length !== 1 ? 's' : ''} — ranked by Opportunity Score</div>
          </div>
          {results
                ...
                <div key={opp.id || i}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-white/80 leading-snug line-clamp-2">{opp.product?.title || opp.keyword}</div>
                  ...
                  <div className="flex items-center justify-between gap-2 mt-2.5 pt-2.5 border-t border-white/8 ml-9">
