    ...
    return (
      <div>
        <div className="flex items-center gap-3 mb-6 animate-card-in">
          ...
          <span className="text-sm font-semibold text-white/80">{meta.flag} {meta.label}</span>
        </div>

        {list.length === 0 ? (
          <div className="card-dark p-12 text-center">
            <div className="text-4xl mb-3">{meta.flag}</div>
            <p className="font-medium text-white/60 mb-1">No opportunities yet for {meta.label}</p>
            <p className="text-sm text-white/40">
              <Link href="/opportunities" className="text-indigo-400 hover:underline font-medium">Run a search</Link> to discover opportunities here
            </p>
          </div>
        ) : (
          <div className="card-dark overflow-hidden animate-card-in stagger-1">
            <div className="px-4 sm:px-6 py-3.5 border-b border-white/8 bg-white/5 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <span className="text-xl">{meta.flag}</span>
                <span className="font-semibold text-white/80">{meta.label}</span>
                <span className="text-xs leading-none text-white/50 bg-white/5 border border-white/10 px-2 py-1 rounded-full">
              ...
              {list.map((opp: any) => (
                <Link key={opp.id} href={`/opportunities/${opp.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-white/80 truncate">{opp.product?.title}</div>
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full text-sm min-w-[520px]">
                <thead className="bg-white/5 text-xs text-white/40 border-b border-white/8">
                  <tr>
                    ...
                    <tr key={opp.id} className="hover:bg-white/5 transition-colors">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="font-medium text-white/80 line-clamp-1">{opp.product?.title}</div>
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/opportunities/${opp.id}`}
                          className="text-xs text-indigo-400 hover:underline font-semibold whitespace-nowrap">
                          View →
                        </Link>
                      </td>
                    </tr>
  ...
  return (
    <div>
      ...
      <div className="mb-6 animate-card-in">
        ...
        <h1 className="text-2xl font-black text-white">Global Marketplace Map</h1>
          ...
          <button key={r} onClick={() => setRegionFilter(r)}
            className={`shrink-0 px-3 py-2 rounded-full text-xs font-medium leading-none transition-colors whitespace-nowrap ${
              regionFilter === r
                ...
                : 'bg-white/5 text-white/50 border border-white/10 hover:border-white/15 hover:text-indigo-400'
            }`}>
            {r}
          </button>
        ))}
          ...
          {[1, 2, 3].map(i => (
            <div key={i} className="card-dark p-5 animate-pulse">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                {[1,2,3,4,5].map(j => <div key={j} className="h-20 bg-white/8 rounded-xl" />)}
              </div>
            </div>
          ))}
        </div>
          ...
          {filteredRegions.map((region, ri) => {
            ...
            return (
              <div key={region} className={`card-dark rounded-xl overflow-hidden animate-card-in stagger-${Math.min(ri + 1, 6)}`}>
                <div className="px-4 sm:px-5 py-3.5 border-b border-white/8 flex items-center justify-between bg-white/5">
                  <span className="font-semibold text-white/70 text-sm">{region}</span>
                  <span className="text-xs text-white/40 bg-white/5 border border-white/10 px-2 py-0.5 rounded-full">{mps.length} marketplace{mps.length !== 1 ? 's' : ''}</span>
                </div>
                    ...
                    return (
                      <button key={mp.id} onClick={() => setSelected(mp.code)}
                        className={`text-left p-3 rounded-xl border transition-all hover:shadow-md active:scale-[0.98] ${
                          mp.active
                            ? 'border-white/10 hover:border-white/15 bg-white/5 hover:bg-indigo-500/10'
                        ...
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {stat ? (
                            <span className={`text-[11px] leading-none font-bold px-1.5 py-1 rounded-full ${
                              stat.avgScore >= 65 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                              stat.avgScore >= 40 ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' :
                              'bg-white/8 text-white/50 border border-white/10'
                            }`}>
                              {stat.avgScore}
                            </span>
                          ) : (
                            <span className="text-xs text-white/30 font-medium leading-none">—</span>
                          )}
                        </div>

                        <div className="text-xs font-semibold text-white/80 leading-snug line-clamp-1 mb-0.5">
                          {meta.label}
                        </div>
      ...
      {/* Summary bar */}
      {!isLoading && (
        <div className="mt-6 p-4 bg-white/5 rounded-xl border border-white/10 flex flex-wrap gap-4 text-sm text-white/50 shadow-sm">
          <span>
            <strong className="text-white/80">{(marketplaces as any[]).filter((m: any) => m.active).length}</strong> active
          </span>
          <span>
            <strong className="text-white/80">{(marketplaces as any[]).filter((m: any) => !m.active).length}</strong> inactive
          </span>
          <span>
            <strong className="text-white/80">{Object.values(oppStats).reduce((a, s) => a + s.count, 0)}</strong> total opportunities
          </span>
          <span className="ml-auto">
            <Link href="/settings" className="text-indigo-400 hover:underline text-xs font-semibold">
              Manage marketplaces →
            </Link>
          </span>
        </div>
