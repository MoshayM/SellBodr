    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      {isLoading ? (
        <div className="flex items-center justify-center h-48">
          <div className="animate-spin text-3xl text-indigo-500">⟳</div>
        </div>
        ...
        <div className="sm:hidden space-y-3">
          {filtered.map((opp: any, idx: number) => {
                  ...
                  <tr key={opp.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <Link href={`/opportunities/${opp.id}`}
                          className="text-xs px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-colors whitespace-nowrap">
                          Full Report
                        </Link>
                        <button onClick={() => handleRemove(opp.id)}
