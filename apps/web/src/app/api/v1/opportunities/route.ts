...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = result.rows.map(r => ({
      ...
      score: r.sOpportunity != null
        ? { opportunity: r.sOpportunity, demand: r.sDemand, competition: r.sCompetition, margin: r.sMargin, trend: r.sTrend, shipping: r.sShipping }
        : null,
      profitModel: r.pmNetProfit != null
        ? { netProfitMinor: r.pmNetProfit, netMarginPct: r.pmNetMargin }
        : null,
    }));
